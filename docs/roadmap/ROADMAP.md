...
| `DETECTION-ADAPTIVE-SAMPLE-SIZE` | Decouple the partial-hash sample size from the small-file threshold (and/or cap it well below 128 KiB) so large files aren't over-sampled during pruning | `DETECTION-BENCHMARK-VS-FCLONES` | `FCLONE-DETECTION-001` (revision) | `few_large_duplicates` benchmark scenario matches or beats fclones | Not Started | motivated by `docs/benchmarks/FCLONES-COMPARISON.md` — rusty_fclone currently loses this scenario ~1.2x |
